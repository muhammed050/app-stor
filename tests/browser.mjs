...
atomic(() =>
  post(client.id, 20000, 0, "Test fixture credit", "fixture", "fixture-credit"),
);
save("publisher", {
  id: `publisher:${pub.id}`,
  owner: pub.id,
...
try {
  ...
  await page.getByRole("button", { name: "صاحب حساب" }).waitFor();
  assert.equal(
    await page
  ...
  await page.getByRole("link", { name: "طلب نشر جديد", exact: true }).click();
  ...
  await page.getByLabel("رقم الإصدار", { exact: true }).fill("1.0.0");
  await page
  ...
  await page.getByRole("button", { name: "إرسال التطبيق للفحص" }).click();
  await page
    .getByRole("heading", { name: "دفتر المهام", exact: true })
    .waitFor();
  await page.getByText("قيد الفحص", { exact: true }).waitFor();
  const appPath = new URL(page.url()).pathname;
  await page.goto(base + "/wallet");
  await page.getByRole("heading", { name: "المحفظة", exact: true }).waitFor();
  await page.getByText("حجز رسم فحص التطبيق", { exact: true }).waitFor();
  await page.getByRole("button", { name: "تسجيل الخروج" }).click();
  await login("admin@example.test");
  await page.goto(base + "/admin");
  await page.getByRole("heading", { name: "نظرة شاملة على إلديفو" }).waitFor();
  await page.screenshot({
  ...
  await page.getByRole("button", { name: "اعتماد قرار الفحص" }).click();
  await page.getByText("بانتظار ناشر", { exact: true }).waitFor();
  await page.getByRole("button", { name: "تسجيل الخروج" }).click();
  await login("publisher@example.test");
  await page.goto(base + "/market");
  await page.getByRole("button", { name: "راجع وقدم عرضك" }).click();
  await page
    .getByLabel("رسالتك لصاحب التطبيق")
    .fill("جاهز لنشر التطبيق بعد الاتفاق ومراجعة ملف الإصدار.");
  await page.getByRole("button", { name: "تقديم عرض النشر" }).click();
  await page.getByRole("button", { name: "تم تقديم عرضك" }).waitFor();
  await page.getByRole("button", { name: "تسجيل الخروج" }).click();
  await login("client@example.test");
  await page.goto(base + appPath);
  await page.getByRole("button", { name: "اختيار", exact: true }).click();
  await page.getByRole("button", { name: /تأكيد وحجز/ }).click();
  await page.getByText("جارٍ النشر", { exact: true }).waitFor();
  await page
  ...
  await page.getByText("الدفع غير متاح حاليًا.", { exact: false }).waitFor();
  ...
  await page
    ...
    .getByRole("link", { name: "المحفظة", exact: true })
    .click();
  await page.getByRole("heading", { name: "المحفظة", exact: true }).waitFor();
  assert.equal(
    await page.evaluate(
