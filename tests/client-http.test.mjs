...
test("API failures show actionable errors without exposing proxy HTML", async (t) => {
  ...
  for (const status of [200, 404, 502, 503]) {
    mock.mock.mockImplementation(
      async () => new Response("The page could not be found", { status }),
    );
  ...
  mock.mock.mockImplementation(async () =>
    Response.json({ error: "رصيد غير كافٍ" }, { status: 400 }),
  );
  await assert.rejects(
    requestJson("/api/apps", { method: "POST" }),
    /رصيد غير كافٍ/,
  );
  mock.mock.mockImplementation(async () => Response.json({ user: null }));
  assert.deepEqual(await requestJson("/api/me"), { user: null });
  mock.mock.mockImplementation(async () => Response.json("app-id"));
  assert.equal(await requestJson("/api/apps/app-id/offer", { method: "POST" }), "app-id");
});
