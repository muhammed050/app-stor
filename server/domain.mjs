...
export function owned(key, u, kind) {
  const r = record(key);
  if (
    !r ||
    (kind &&
      !db.prepare("SELECT 1 FROM records WHERE id=? AND kind=?").get(key, kind))
  )
...
export function timeline(a, label) {
  a.history ||= [];
  a.history.push({ label, at: now() });
}
const activeUser = (id) =>
  db.prepare("SELECT * FROM users WHERE id=? AND status=?").get(id, "active");
function fileOwned(id, u, type) {
  const f = record(id);
  if (!f || f.owner !== u.id || f.kind !== type)
    fail("ارفع الملف المطلوب أولًا");
  return f;
}
function platform(delta, label, ref, key) {
  post("platform", delta, 0, label, ref, key);
}
function snapshot(s) {
...
export function state(u) {
  const admin = u.role === "admin",
    apps = records("app");
  const mine = apps.filter(
    (a) => admin || a.owner === u.id || a.publisherId === u.id,
  );
  const publishers = records("publisher");
  return {
    user: cleanUser(u),
    settings: settings(),
    balance: balance(u.id),
    ledger: db
      .prepare(
    ...
    market:
      u.role === "publisher" &&
      record(`publisher:${u.id}`)?.status === "approved"
        ? apps
        ...
        : [],
    publisher: record(`publisher:${u.id}`),
    publishers: admin
      ? publishers
      : publishers
          .filter((p) =>
    ...
    payments: records("payment").filter((r) => admin || r.owner === u.id),
    withdrawals: records("withdrawal").filter((r) => admin || r.owner === u.id),
    tickets: records("ticket").filter((r) => admin || r.owner === u.id),
    updates: records("update").filter(
      (r) => admin || r.owner === u.id || r.publisherId === u.id,
    ),
    notifications: records("notification")
    ...
    audit: admin
      ? db
          ...
          .all()
      : [],
    platformBalance: admin ? balance("platform") : undefined,
...
export function createApp(u, b) {
  role(u, "client");
  const s = settings();
  const f = fileOwned(b.fileId, u, "app");
  ...
  if (
    records("app").some(
      (a) =>
        a.packageName === pkg && !["cancelled", "rejected"].includes(a.status),
    )
  )
  ...
  return atomic(() => {
    const a = save("app", {
      ...
      price: {
        ...snapshot(s),
        publishFee: budget,
        publisherShare: budget - Math.round((budget * s.commissionBps) / 10000),
    ...
    audit(u.id, "app.create", a.id);
    return save("app", a);
  });
}
export function publisherProfile(u, b) {
  role(u, "publisher");
  const existing = record(`publisher:${u.id}`);
  if (
  ...
  audit(u.id, "publisher.submit", p.id);
  return p;
}
export async function verifyPublisher(u) {
  role(u, "publisher");
  const p = owned(`publisher:${u.id}`, u, "publisher");
  ...
  const fresh = owned(p.id, u, "publisher");
  if (fresh.challenge !== p.challenge)
    fail("تغير طلب التحقق. أعد المحاولة.", 409);
  fresh.verification = result;
  save("publisher", fresh);
  audit(u.id, "publisher.verify", p.id, result.status);
  return result;
}
export function offer(u, key, b) {
  role(u, "publisher");
  const p = record(`publisher:${u.id}`);
...
export function choose(u, key, b) {
  role(u, "client");
  return atomic(() => {
    const a = owned(key, u, "app");
    if (
      a.status !== "open" ||
      !a.offers.some((o) => o.publisherId === b.publisherId)
    )
      fail("العرض غير متاح");
    const p = record(`publisher:${b.publisherId}`);
    if (p?.status !== "approved" || !activeUser(b.publisherId))
    ...
    save("app", a);
    notify(a.publisherId, "تم اختيار عرضك", a.title, `/apps/${a.id}`);
    audit(u.id, "offer.accept", a.id);
    return a;
  });
}
export function submitLink(u, key, b) {
  role(u, "publisher");
  const a = owned(key, u, "app");
...
export async function verifyApp(u, key) {
  ...
  return atomic(() => {
    ...
    save("app", fresh);
    audit(u.id, "publication.verify", key, result.status);
    return result;
  });
}
export function settle(key, actor = "system") {
  return atomic(() => {
    const a = record(key);
    if (
      !a ||
    ...
    if (
      ...
      record(`publisher:${a.publisherId}`)?.status !== "approved"
    )
      fail("يلزم تحقق حديث وحساب ناشر معتمد", 409);
    if (!activeUser(a.owner) || !activeUser(a.publisherId))
    ...
    save("app", a);
    audit(actor, "publication.settle", key);
    notify(a.publisherId, "أرباح جديدة متاحة", a.title, "/wallet");
    return a;
  });
}
export function message(u, key, b) {
  const a = owned(key, u, "app");
  a.messages ||= [];
  a.messages.push({
...
export function dispute(u, key, b) {
  const a = owned(key, u, "app");
  ...
  save("app", a);
  audit(u.id, "dispute.open", key);
  return a;
}
export function renew(u, key) {
  role(u, "client");
  return atomic(() => {
    const a = owned(key, u, "app");
    if (a.status !== "completed") fail("التطبيق لم يكتمل نشره");
    if (!activeUser(a.publisherId)) fail("حساب الناشر غير متاح");
...
export function createUpdate(u, key, b) {
  role(u, "client");
  return atomic(() => {
    const a = owned(key, u, "app");
    ...
    notify(a.publisherId, "طلب تحديث جديد", a.title, `/apps/${key}`);
    return update;
  });
}
export function updateAction(u, key, b) {
  return atomic(() => {
    const r = owned(key, u, "update");
    if (b.action === "submit") {
      ...
      platform(r.fee - r.share, "عمولة تحديث", key, `update-platform:${key}`);
      r.status = "completed";
    ...
    audit(u.id, `update.${b.action}`, key);
    return save("update", r);
  });
}
export function payment(u, b) {
  if (!settings().depositsEnabled) fail("الشحن غير متاح حاليًا");
  const f = fileOwned(b.fileId, u, "receipt");
  return save("payment", {
    owner: u.id,
    name: u.name,
...
export function adminAction(u, kind, key, b) {
  role(u, "admin");
  return atomic(() => {
    if (kind === "users") {
      if (key === u.id) fail("لا يمكن تعليق حسابك الحالي");
      const target = db.prepare("SELECT * FROM users WHERE id=?").get(key);
      if (!target || target.role === "admin") fail("المستخدم غير متاح");
      if (!["active", "suspended"].includes(b.status)) fail("حالة غير صالحة");
      db.prepare("UPDATE users SET status=? WHERE id=?").run(b.status, key);
      if (b.status === "suspended")
        db.prepare("DELETE FROM sessions WHERE user_id=?").run(key);
      audit(u.id, "user.status", key, b.status);
      return {};
    }
    if (kind === "settings") {
      const s = settings();
      ...
      save("settings", { ...s, id: "settings" });
      audit(u.id, "settings.update", "settings");
      return s;
    }
    const map = {
    ...
    if (kind === "publishers") {
      ...
      if (b.action === "review") {
        ...
        if (!["approve", "reject"].includes(b.decision)) fail("قرار غير صالح");
        post(
          r.owner,
          0,
          -r.price.reviewFee,
        ...
        platform(r.price.reviewFee, "رسوم فحص", key, `review-platform:${key}`);
      ...
      notify(r.owner, "تحديث على طلبك", r.title, `/apps/${key}`);
      ...
      if (b.action === "approve") {
        if (
          records("payment").some(
            (p) =>
              p.status === "approved" && p.adminReference === r.adminReference,
          )
        )
      ...
      if (b.action === "approve") {
        ...
        if (!activeUser(r.owner)) fail("الحساب معلق ولا يمكن تنفيذ السحب");
        if (!/^(0x)?[0-9a-fA-F]{64}$/.test(r.adminReference))
          fail("أدخل معرّف معاملة blockchain صالحًا");
        if (
          records("withdrawal").some(
            (w) => w.status === "paid" && w.adminReference === r.adminReference,
          )
        )
          fail("مرجع التحويل مستخدم");
        platform(r.fee || 0, "رسوم سحب", key, `withdraw-fee:${key}`);
        post(
          r.owner,
          0,
          -r.amount,
      ...
      notify(r.owner, "تحديث طلب السحب", r.adminReference, "/wallet");
    } else if (kind === "tickets") {
      r.reply = text(b.reply, 3, 4000);
      r.status = b.close ? "closed" : "open";
      notify(r.owner, "رد على طلب الدعم", r.subject, "/support");
