...
export async function createCheckout(u, b) {
  ...
  if (cents < settings().minTopup) fail("المبلغ أقل من الحد الأدنى للدفع");
  const previous = records("payment").find(
    (p) =>
      p.owner === u.id &&
      p.provider === "whop" &&
      p.status === "pending" &&
    ...
    const r = await fetch(
      "https://api.whop.com/api/v1/checkout_configurations",
      {
        ...
        body: JSON.stringify({
          ...
          metadata: { eldevo_payment_id: p.id, eldevo_user_id: u.id },
...
export function applyEvent(event) {
  ...
  if (!payment?.id) return { ignored: true };
  if (event.type === "payment.succeeded")
    return atomic(() => {
      const p = record(payment.metadata?.eldevo_payment_id);
      if (!p || p.provider !== "whop") return { ignored: true };
      if (p.status === "approved") return { duplicate: true };
      ...
      save("payment", p);
      audit("whop", "payment.succeeded", p.id, payment.id);
      notify(
        p.owner,
        "تم تأكيد الدفع",
  ...
  if (/refund|dispute|chargeback/.test(event.type)) {
    ...
    const p = records("payment").find((p) => p.providerPaymentId === paymentId);
    if (p)
      atomic(() => {
        db.prepare("UPDATE users SET status='suspended' WHERE id=?").run(
          p.owner,
        );
        db.prepare("DELETE FROM sessions WHERE user_id=?").run(p.owner);
        for (const app of records("app").filter(
          (a) =>
            a.owner === p.owner &&
        ...
        save("payment", p);
        audit("whop", "payment.risk", p.id, event.type);
      });
