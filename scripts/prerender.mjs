...
for (const [path,page] of Object.entries(publicPages)) {
  ...
  const tags = `<link rel="canonical" href="${origin+path}"/><meta property="og:type" content="website"/><meta property="og:site_name" content="إلديفو"/><meta property="og:locale" content="ar_AR"/><meta property="og:title" content="${escape(page.title)}"/><meta property="og:description" content="${escape(page.description)}"/><meta property="og:url" content="${origin+path}"/><meta property="og:image" content="${origin}/brand/eldevo-logo.png"/><meta property="og:image:alt" content="شعار إلديفو"/><meta name="twitter:card" content="summary"/><script id="site-schema" type="application/ld+json">${JSON.stringify(pageSchema(path)).replaceAll('<','\\u003c')}</script>`;
...
const urls = Object.keys(publicPages).map(path=>`<url><loc>${origin+path}</loc>${path==='/'?`<image:image><image:loc>${origin}/brand/eldevo-logo.png</image:loc><image:title>شعار إلديفو — Eldevo</image:title></image:image>`:''}</url>`).join('\n');
...
await writeFile('dist/404.html',template.replace('<title>إلديفو — مساحة العمل</title>','<title>الصفحة غير موجودة | إلديفو</title>').replace('<div id="root"></div>','<div id="root"><main class="fatal"><img src="/brand/eldevo-mark.webp" alt="إلديفو" width="64" height="64"/><h1>الصفحة غير موجودة</h1><a href="/" class="button primary">العودة للرئيسية</a></main></div>'));
